...
function Nav() {
  ...
  const fetchBalance = async () => {
    try {
      ...
      const finalbalance = web3.utils.fromWei(balanceWei, "ether") + " " + networks["Etherlink"]["nativeCurrency"]["name"];
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Try to get chainId, but handle the case where it's not supported
      let chainId;
      try {
        chainId = await web3.eth.getChainId();
      } catch (error) {
        console.log("ChainId method not supported, proceeding with network switch");
        chainId = null;
      }

      const EtherlinkChainId = parseInt(networks.Etherlink.chainId, 16);

      // If we got a chainId and it doesn't match, switch networks
      if (chainId && parseInt(chainId) !== EtherlinkChainId) {
        try {
